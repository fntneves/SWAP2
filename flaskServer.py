...
def solver(jsonObject):
    ...
    if solucao != solver.OPTIMAL:
        ...
        # escrever objecto enrollments que vai ser returnado
        lista_enrollments = [] 
        for al in r:
            for uc in r[al]:
                if uc[-2:] != suf_teoria and uc[:6] != 'ALMOCO':
                    lista_enrollments.append({ 'student_id':al, 'course_id':uc, 'shift_id':r[al][uc][0] })
        # return { 'enrollments': lista_enrollments }
        return lista_enrollments
