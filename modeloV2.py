...
# carrega as capacidades iniciais
    ...
    for dia in elem:
        for uc in elem[dia]:
            for codigo in uc:                 
                ...
                if turno not in capacidades[codigo]:
                    capacidades[codigo][turno] = [uc[codigo]['Capacidade']]
                else:
                    capacidades[codigo][turno] += [uc[codigo]['Capacidade']]
...
# ciclo while que a partir daqui vai limitar os alunos e vai reproduzir o codigo a partir daqui
n_alunoAtual = 0
novosAlunos = alunos.copy()
while(n_alunoAtual < len(novosAlunos)):
    for al in alunos:
        if n_alunoAtual == int(sys.argv[4]):
            n_alunoAtual = 0
            break
    ...
    capacidade_maxima_TP_c = [ Sum(lista_capacidades[uc][turno]) <= capacidades[uc][turno][i] for uc in lista_capacidades for turno in lista_capacidades[uc] for i in range(len(capacidades[uc][turno])) if uc[-2:] != suf_teoria ]

    capacidade_maxima_T_c = [ Sum(lista_capacidades[uc][turno]) <= capacidades[uc][turno][i] for uc in lista_capacidades for turno in lista_capacidades[uc] for i in range(len(capacidades[uc][turno])) if uc[-2:] == suf_teoria ]
    ...
    #         grupos_c += [ And([ presencas[al1][uc][t] == presencas[al][uc][t] for al in grupos_json[uc][grupo] if al != al1 ]) for t in presencas[al1][uc] ]
    #         sum_grupos += [ If(And([ presencas[al1][uc][t] == presencas[al][uc][t] for al in grupos_json[uc][grupo] if al != al1 ]),1,0) for t in presencas[al1][uc] ]
    ...
    for uc in capacidades:
        for turno in capacidades[uc]:
            for i in range(len(capacidades[uc][turno])):
                if uc in alocacoes_finais and turno in alocacoes_finais[uc]:
                    capacidades[uc][turno][i] -= alocacoes_finais[uc][turno]
    #atualizar os alunos e limpar variaveis
    alunos = novosAlunos.copy()
    presencas = {}
